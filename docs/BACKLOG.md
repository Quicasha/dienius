...
| **Another recipe on a meal in two presses.** Putting another recipe of the same meal on a meal of the day takes five presses - its menu, Details, the recipe line, the recipe, Done - and six once it is ticked and folded under Done | The owner's day, pressed through, 2026-09-25 (e2e/owners-day.e2e.ts counts it) | A press on the meal's recipe opens Kitchen today, and is kept for that; the honest shape is a second control beside it on the card - "another" - opening the meal's other recipes in place, a press to choose. A new control on the most-read card, checked by the sweep and the precision pass at both sizes: half a day. Every other part of the ordinary day is two presses or fewer, and the next meal is on the first screen at 375px |
| **A kind before a day shift.** The free day before a day shift keeps a free day's evening, which runs into the day shift's early night | The owner's month, lived in a test, 2026-09-25 (docs/OPEN-QUESTIONS.md) | A rule beside `afterNight`: a kind naming the kind it is on the day before another, read at every door `resolveAfterNight` is - a field on the mark, the resolver both ways, the editor's select, the file's field and v2.40's tests again, about a day. Until then the file's evening block can end by the day shift's bedtime |
