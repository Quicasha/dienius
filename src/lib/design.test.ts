...
const RETIRED: { what: string; now: () => number; left: number }[] = [
  ...
  { what: 'font weights written as numbers', now: () => count(/font-weight\s*:\s*[0-9]+/g), left: 75 },
  {
    what: 'line heights written as numbers',
    now: () => count(/line-height\s*:\s*(?!1\s*[;}\s]|inherit|normal|var\()[0-9.]+(px)?/g),
    left: 78,
  },
  { what: 'tracked capitals', now: () => count(/text-transform\s*:\s*uppercase/g), left: 15 },
  { what: 'borders drawn', now: drawnBorders, left: 106 },
  { what: 'black written as rgba()', now: () => count(/rgba\(0,\s*0,\s*0,/g), left: 8 },
  { what: 'presses that scale', now: pressesThatScale, left: 0 },
  { what: 'heights written as min-height in pixels', now: () => count(/min-height\s*:\s*[0-9]+px/g), left: 60 },
]
