...
/**
 * Later merges per item, under its wire name `backlog` - see LaterItem. Two
 * devices adding to it on the same evening must both keep what they added:
 * this is the list somebody reaches for when a day has room, and losing half
 * of it to a merge would be the one failure that makes them stop trusting it.
 */
test('a Later item added on the phone and one added on the PC both survive', () => {
