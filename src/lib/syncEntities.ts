import type { AppData, Category, DayPlan, Goal, IfThenEntry, InboxItem, LaterItem, LibraryItem, LibraryList, Picture, Recipe, Routine, ScratchNote, Settings, Task, Template } from './types'
...
const KINDS: EntityKind[] = ['task', 'day', 'template', 'list', 'item', 'goal', 'ifthen', 'inbox', 'backlog', 'scratch', 'category', 'picture', 'recipe', 'routine', 'setting']
...
export function collectEntities(data: AppData): Map<EntityKey, Entity> {
  ...
  // An entity of its own rather than a settings field, which is the whole
  // reason the list lives in AppData - see the note on AppData.categories.
  // Renaming Health on the laptop and recolouring Meals on the phone are two
  // edits to two things, and at this grain they both survive.
  for (const category of data.categories) {
    out.set(keyFor('category', category.id), {
      key: keyFor('category', category.id),
      kind: 'category',
      ref: category,
      bodyOf: () => body(category, 'updatedAt'),
  ...
  // A routine is its own entity too: a gym time moved on the phone and a
  // reading routine added on the PC are two edits to two things.
  for (const routine of data.routines) {
    out.set(keyFor('routine', routine.id), {
      key: keyFor('routine', routine.id),
      kind: 'routine',
      ref: routine,
      bodyOf: () => body(routine, 'updatedAt'),
...
export function stampChanges(previous: AppData, next: AppData, now: string): AppData {
  ...
  diffList('routine', previous.routines, next.routines, changed, removed)
...
function applyStamps(data: AppData, changed: Set<EntityKey>, removed: EntityKey[], now: string): AppData {
  ...
  return {
    ...
    routines: mapIfChanged<Routine>(data.routines, r => touched('routine', r.id), now),
    settingsUpdatedAt,
