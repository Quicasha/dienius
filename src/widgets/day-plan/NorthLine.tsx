 ...
 *
 * Where North has a text, one line of it - a line written under one of its
 * headings, picked for the date and the part of the day by
 * lib/northLine.ts - whole, wrapping rather than cut, and the signature
 * under it, quieter, always. A press anywhere on it opens North. Since
 * v2.26 this is what the day's top carries: the goal's name that stood here
 * was a label written once and cut off at the width of the row, and the
 * text is the part written to be read.
 *
 * The goal's name stands here only where the text has nothing for the day -
 * no text, or a text with no line under a heading and no signature - and
 * where the switch under Nudges has taken North's text off the day.
 */
export function NorthLine({ date, onOpenNorth }: NorthLineProps) {
  const data = useAppData()
  const text = data.picture?.text ?? ''
  if (data.settings.north.stripOnDay !== false && hasSomethingForTheDay(text)) {
    return <NorthTextLine text={text} date={date} onOpenNorth={onOpenNorth} />
  }
  return <NorthGoalLine date={date} onOpenNorth={onOpenNorth} />
}

/** A signature, or a line written under any heading, tagged or not. */
function hasSomethingForTheDay(text: string): boolean {
  const { sections, signature } = parseNorth(text)
  return signature.length > 0 || sections.some(section => section.paragraphs.length > 0)
}
...
function NorthTextLine({ text, date, onOpenNorth }: { text: string; date: string; onOpenNorth: () => void }) {
  ...
  return (
    <div className="north-line is-text" data-tour="north-line">
...
/**
 * The goal's line, where the text has nothing for the day: one line of
 * quiet text under the header, the thing the days are for.
 *
 ...
 */
function NorthGoalLine({ date, onOpenNorth }: NorthLineProps) {
  const data = useAppData()
