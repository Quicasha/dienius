...
test('announces itself as a labelled dialog named for the task', () => {
  ...
  render(
    <TaskGapOffers task={float('Guitar practice', 20)} tasks={tasks} sleepProfileId={undefined} onPlace={() => {}} onClose={() => {}} />,
  )
...
test('focus lands on the dialog itself on open', () => {
  ...
  render(<TaskGapOffers task={float('Guitar', 20)} tasks={tasks} sleepProfileId={undefined} onPlace={() => {}} onClose={() => {}} />)
...
test('a task that fits several gaps lists each one with time, duration and neighbours', async () => {
  ...
  render(<TaskGapOffers task={float('Guitar', 20)} tasks={tasks} sleepProfileId={undefined} onPlace={onPlace} onClose={() => {}} />)
...
test('a task already anchored says so plainly', () => {
  ...
  render(<TaskGapOffers task={anchor('Meeting', '09:00', 60)} tasks={tasks} sleepProfileId={undefined} onPlace={() => {}} onClose={() => {}} />)
...
test('more than four gaps shows only four, with a way to reveal the rest', async () => {
  ...
  render(<TaskGapOffers task={float('Guitar', 10)} tasks={tasks} sleepProfileId={undefined} onPlace={() => {}} onClose={() => {}} />)
...
test('the explicit close button calls onClose', async () => {
  ...
  const onClose = vi.fn()
  const tasks = [float('Guitar', 20)]
  render(<TaskGapOffers task={float('Guitar', 20)} tasks={tasks} sleepProfileId={undefined} onPlace={() => {}} onClose={onClose} />)
...
test('tapping the scrim behind the sheet calls onClose', async () => {
  ...
  const { container } = render(
    <TaskGapOffers task={float('Guitar', 20)} tasks={tasks} sleepProfileId={undefined} onPlace={() => {}} onClose={onClose} />,
  )
