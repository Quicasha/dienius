...
(function () {
  ...
  window.__audit = function audit(label) {
    ...
    // panel sits over the day-type buttons, and a recipe's meals over the card
    // under it, which is where a popover goes.
    const layer =
      document.querySelector('[role="dialog"], .replan, .task-detail-panel, .task-detail-sheet, .scratch-overlay, .task-gap-offers-panel, .task-actions-sheet, .swatch-picker-panel, .meals-picker-panel') || null
